...
void loop() {
  ...
  if (dht.getData() && counter % 10 == 0) {    // get All data from DHT11 every 10 seconds
  ...
  counter += 1;
  // We want the fan to turn on for 15 minutes every 30 minutes
  if (counter <= timeOn * 60) {
    analogWrite(FAN, 256); //full power is 256
